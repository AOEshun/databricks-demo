...
42. As a future maintainer, I want `DW_<TABEL>` deletes recorded by `__END_AT`-setting (not as tombstone rows), so that the `APPLY CHANGES INTO STORED AS SCD TYPE 2` mechanism doesn't have to be sidestepped — per ADR-0010's documented trade-off.
...
44. As a future maintainer, I want `WA_HASH` computed in the tagged source MV before the `APPLY CHANGES INTO` step (so DWQ rows carry it too, per ADR-0015), so that source-reconciliation queries can cover rejected rows on the same footing as cleansed rows.
...
- **`DW_<TABEL>` streaming table** — defined with `WK_<TABEL> BIGINT GENERATED ALWAYS AS IDENTITY`, full business-column list, the five `WA_*` admin columns, and `EXPECT (...) ON VIOLATION FAIL UPDATE` declarations for fail-grade rules (e.g. `order_id IS NOT NULL`). Populated by `APPLY CHANGES INTO <tagged source MV> KEYS (<BK>) SEQUENCE BY _commit_timestamp APPLY AS DELETE WHEN WA_CRUD = 'D' STORED AS SCD TYPE 2`. The clean-row predicate is `size(failed_rules) = 0`, applied in the `APPLY CHANGES INTO` source projection.
...
- **`DWQ_<TABEL>` streaming table** — populated from the same tagged source MV with `WHERE size(failed_rules) > 0`. Carries every business and `WA_*` column from the rejected change event plus `failed_rules`. No `WK_<TABEL>` IDENTITY, no `__START_AT` / `__END_AT`, append-only (no `APPLY CHANGES`). DWQ is allowed to retain the rejected source's `_change_type` for diagnostic purposes.
...
- **`DW_<TABEL>` + `DWH_<TABEL>` pair pattern** — IDENTITY `WK_<TABEL>`, `APPLY CHANGES INTO ... STORED AS SCD TYPE 2`, then a deterministic view projection that renames validity columns and computes `WKP_` / `WKR_`. One template per entity, varying BK and business columns.
